...
### 步骤 0.5 — 读取 2026 政策基线与学区/预警源（新增）

在发现小区前，先拉取政策与学区权威源，作为后续所有判断的输入：

```bash
# 2026 政策基线（多校划片/教师轮岗/户籍脱钩/学位锁定/预警）—— 默认高权重情景输入
python scripts/data_sources.py policy --city <城市>

# 学区划片源 + 学区预警源（C+D 模块）：区教育局招生专栏 + 对口地段表 + 红黄牌预警
python scripts/data_sources.py sources --city <城市>
```

- **policy 基线**：将返回的「多校划片 / 教师轮岗 / 户籍脱钩 / 学位锁定年限 / 预警机制」作为 cohort 分析与价格预测的默认高权重输入；凡 `verify_status=待联网核验` 的城市，以当年教育局公告为准，勿直接引用为结论。
- **school_district 源**：区教育局招生专栏 URL、发布月份（通常 4-5 月）、是否可下载对口表、解析提示；取数时**必须取当年版本**，禁止引用往年。
- **enrollment_alert 源**：学区预警（黄/橙/红）发布入口与等级口径；预警直接拉低学区确定性，应进入溢价与三情景的「政策效应」因子，引用时保留官方免责声明。

### 步骤 0.6 — 学位占用 / 落户年限（买前用户自查，skill 不机读）

学区确定性不仅取决于对口学校，还取决于「该房产的学位是否被占用、落户是否满足年限」。这两项 **skill 无法机读具体房产状态**（属政务 App 个人隐私查询），必须在报告结论前提示用户自行核验：

- 按 `references/title-verification-checklist.md` 的「买前 3 步自查」引导用户：① 政务 App 查学位占用 / 五年一户；② 区教育局/对口校线下核验；③ 合同加《学位未被占用声明书》+ 赔偿条款。
- 报告「学区确定性」章节须含**醒目提示框**：本分析仅研究城市政策规则，不构成对该房产具体学位/落户状态的确认。
...
**合规优先**：取数首选官方开放平台 API（贝壳开放平台 OAuth2.0）或已公开单小区数据的城市官方源（`gov` 命令直拉 T1）；个人对核心接口抓包/破解验签存在刑事风险（已有判例），详见 `data-source-playbook.md` 第十节「合规红线」。
