...
5. 采集数据：按 `references/data-source-playbook.md` 执行，覆盖成交、挂牌、库存、政策、城市基本面和可比楼盘。**先做透第一维度「房价」**（挂牌/成交月度时间轴 + 环比/同比/N月涨跌幅 + 带看/房源量），再按用户诉求逐层展开成交量/供需比/土地出让/学区政策/人口流动/信贷环境（见 `references/dimension-network.md`，脚本 `python scripts/data_sources.py dimensions` 可查各维度字段与来源）。**核心双源：贝壳系（贝壳/链家）+ 我爱我家，关键数据（成交价/挂牌价/小区档案）尽量双平台交叉核对**；杭州叠加杭房数研/小鸡选房高频源；诸葛找房/安居客/房天下/58同城仅作交叉验证。
   ...
   - **成交数据轻量获取（CLI 无成交时，保持轻量、绝不编造）**：贝壳 CLI 后端已下架 `buy sold` / `buy market`；纯网页渠道直连抓取（链家/贝壳成交页验证码墙、透明售房网/房天下 JS 渲染、安居客/58 验证码墙）**均不可行**，**不要挂 Playwright / CDP 等重方案**。**主路径 = 联网检索**：agent 用 WebSearch 以「<社区> <城市> 成交记录 / 参考成交均价 / 最近成交」等精准查询，检索房天下/链家/我爱我家/安居客等**已被搜索引擎索引**的成交与参考均价；**真实命中即带入报告，严格标注来源 + 口径（成交/参考均价）+ 日期，绝不编造；检索无果才标「暂不可得」**。**补充路径 = 录入**：若用户在贝壳/链家 App 或小程序复制了近期成交（或 agent 把 WebSearch 命中的成交转录），保存为 `output/chengjiao_<社区>.txt`（首行可写 `# 来源: 房天下（网络检索）` 声明来源；每行一条：日期/面积/总价/户型，字段顺序宽松；也支持 Markdown 表格），重跑 `python scripts/gen_styled_report.py --community <社区> --city <城市> --chengjiao output/chengjiao_<社区>.txt`。脚本零依赖解析，按声明来源标注「录入：<来源>」，无官方详情链接、提示自行核验。
