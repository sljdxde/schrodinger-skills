...
6. 补齐教育与社区：涉及学区房或用户提到孩子入学时，必须按 `references/school-and-community-analysis.md` 采集学校升学情况、学校生源、小区人口与居住画像，并按 `references/school-tier-reference.md` 给出目标学校**梯队定位（第一/二/三/四梯队 + 评级依据）**。**初中升学率必须给近 3-5 年数据，并按 `references/school-cohort-analysis.md` 做「生源代际传导分析」**：升学率滞后约 9-15 年，须重建历史升学率对应的当年生源、对比当前生源，预测目标入学批次孩子未来升学表现（如 2028 入学 → 2034 初中 → 2037 中考），给出数量效应/质量效应/学校效应/政策效应四因素拆解与三情景预判。
