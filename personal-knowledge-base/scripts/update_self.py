#!/usr/bin/env python3
"""Self-update helper for the personal-knowledge-base skill bundle.

This bundle contains two sub-skills (`ob-llm-wiki` and `ob`). When run from
inside the `personal-knowledge-base` folder, this script checks the GitHub copy
of the whole bundle folder and can replace the local copy after creating a
recoverable backup. No third-party Python packages are required.

Like the other skills in this repo, the whole bundle folder (including both
sub-skills) is treated as a single updatable unit.
"""
...
SKILL_NAME = "personal-knowledge-base"
REPO_OWNER = "sljdxde"
...
def assert_skill_dir(path: Path) -> None:
    ...
    if not ((path / "SKILL.md").is_file() or (path / "bundle.yaml").is_file()):
        raise RuntimeError(f"Missing SKILL.md or bundle.yaml in {path}")
...
def self_test() -> int:
    with tempfile.TemporaryDirectory(prefix=f"{SKILL_NAME}-test-") as tmp_name:
        ...
        a.mkdir()
        b.mkdir()
        (a / "bundle.yaml").write_text("name: x\n", encoding="utf-8")
        (b / "bundle.yaml").write_text("name: y\n", encoding="utf-8")
...
def main() -> int:
    parser = argparse.ArgumentParser(description=f"Check and update the {SKILL_NAME} skill bundle.")
    parser.add_argument("--check", action="store_true", help="Check for available updates and print JSON status.")
    parser.add_argument("--apply", action="store_true", help="Apply available bundle updates after creating a backup.")
