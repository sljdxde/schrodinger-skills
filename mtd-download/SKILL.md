...
- **固定小分块 + 高并发 + 块级容错（对抗 CDN 限速/挂死）**：多线程不再按 `总大小/线程数` 给每个线程切一大块（那样遇到「单连接限速 + 大块 Range 挂死」型 CDN 会直接卡死），而是把文件切成**固定小分块**（默认自动：代理环境 **2MB**、否则 5MB，可用 `--chunk` 调整），由线程池消费任务队列，每块独立下载、独立重试（默认 5 次），**单块失败不影响其它块**。小分块保证在限速生效前就下完，从根本上规避挂死（详见 `references/troubleshooting.md`）。
...
- **下载后完整性校验（关键）**：合并后**逐字节扫描大段连续零字节（零空洞）** + 格式校验（PDF 检查 `%PDF` 头与 `%%EOF` 尾；通用调用 `file` 探测是否下成了 HTML 错误页）。任何「大小对但内容可疑」都判失败并自动补下对应分块，**绝不交付满尺寸但内部损坏的文件**。这专门修复了「多线程被中断留下零空洞、却因只比对大小而交付坏文件」的致命问题。
- **断点续传自愈（--resume）**：重跑 `--resume` 时除了读进度文件，还会**主动扫描已存在文件里的大零空洞并视为未完成块**，进程被 SIGKILL 中断后也能自动补下残缺块，直到内容完整。
- **代理绕过 / 镜像源**：`--noproxy` 清除所有代理变量直连；`--mirror ghproxy` 把 URL 包成 `https://ghproxy.net/<URL>` 直连，内置绕过会截断大响应体的透明代理。代理环境下默认分块自动降到 **2MB**（低于 4.2MB 截断阈值），并给出提速/避截断的提示。
- **GitHub blob 链接归一化**：输入 `github.com/.../blob/...` 页面链接会自动改写为 `raw.githubusercontent.com` 真文件直链，避免下到 HTML 页面。
...
# 调整分块大小（默认自动：代理环境 2MB，否则 5MB；小分块抗 CDN 限速挂死）
python scripts/mtd.py <URL> --chunk 2

# 调整单块重试上限（默认 5 次）
python scripts/mtd.py <URL> --max-retry 8

# 断点续传（中断后用同一命令加 --resume 继续；会自动扫描并补下零空洞残缺块）
...
# 绕过透明代理直连（海外 CDN 大文件被代理截断时使用，如 GitHub 大文件）
python scripts/mtd.py <URL> --noproxy

# 走 ghproxy 镜像直连（等价于 --noproxy 且 URL 包成 https://ghproxy.net/<URL>，内置绕过代理）
python scripts/mtd.py <URL> --mirror ghproxy

# 组合：GitHub 大文件 + 镜像直连（blob 链接会自动转 raw，再经 ghproxy 绕过代理）
...
- **断点续传（`--resume`）**：多线程模式把已完成块记录在 `<输出>.mtd-progress`，重跑时跳过已完成块只补剩余；**`--resume` 还会主动扫描已存在文件里的大零空洞并视为未完成块**，进程被 SIGKILL 中断后也能自愈补下残缺块，直到内容完整。单线程模式用 `curl -C -` 续传循环（`--max-time 600` + 自动重连）兜底断流。输出文件已存在时默认报错，需显式 `--resume` 或 `--overwrite`。
- **重试**：每个小块失败自动重试（默认 5 次，指数退避），全部块重试完仍失败才整体失败。
- **完整性校验（下载后必做）**：合并后扫描大段连续零字节（零空洞）+ 格式校验（PDF `%PDF` 头 / `%%EOF` 尾；通用用 `file` 探测 HTML 错误页）。发现零空洞自动转补下对应分块；格式异常或补下后仍有空洞则删除文件判失败。**绝不会交付满尺寸但内部损坏的文件**。
...
- 支持代理绕过（`--noproxy`）与镜像源（`--mirror ghproxy`），无需手动 `unset` 代理；GitHub `blob` 链接自动转 `raw` 直链。
- **沙箱/透明代理环境必读**：若环境注入了透明代理（会对 >4.2MB 的海外 CDN 响应体截断），下载大文件请加 `--noproxy` 或 `--mirror ghproxy` 直连；且**大文件下载建议放到后台运行**——本沙箱前台长下载约 3 轮后会被 SIGKILL（exit 137），即使中断，重跑 `--resume` 也能自愈补下残缺块。断点续传 `--resume` 与 SHA256 校验已支持，见上。
